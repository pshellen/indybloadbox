...
-- Logo displayed above the poster
...
local function compute_layout()
    ...
    layout.footer_h = math.max(
        short * 0.11,
        layout.bottom_size * 2.8
    )
    ...
    -- Rotated portrait signs may clip the logical bottom edge.
    local safe_inset =
        portrait and math.max(scale_y(100), HEIGHT * 0.10)
        or scale_y(32)

    layout.footer_y =
        HEIGHT - safe_inset - layout.footer_h
    ...
    -- Position and maximum size of logo.png.
    -- Using the short side keeps it consistent across
...
local function draw_top_logo()
    ...
    local logo_w, logo_h = top_logo:size()
...
local function draw_badge(text, upcoming)
    ...
    -- Badge slightly overlaps the top of the poster.
    local y1 =
        layout.poster_y -
        box_h * 0.4

    local fill =
        upcoming and badge_green
        or badge_blue
...
local function draw_title_row(show)
    ...
    if show.is_3d and badge_3d then
        local source_w, source_h = badge_3d:size()

        badge_h = layout.badge_3d_size

        badge_w =
            badge_h *
            (source_w / math.max(source_h, 1))
...
local function draw_bottom_bar(show)
    ...
    -- logo.png is now shown above the poster.
    -- Only the separately configured corner logo
    -- remains in the footer.
    if corner_logo then
        local logo_w, logo_h = corner_logo:size()
...
local function draw_show_info()
    ...
    local show_time =
        (screen.show.start or ""):upper()

    draw_badge(
        screen.show.status_label,
        screen.show.upcoming
    )
...
local function resolve_sign(signs, serial)
    for index = 1, #signs do
        if signs[index].serial == serial then
            return signs[index]
        end
    end
    ...
    if #signs == 1 then
        print(
            "WARNING: device serial " ..
            serial ..
            " not in config; using the only configured sign"
        )
...
util.file_watch("config.json", function(raw)
    ...
    if sign then
        ...
        print(
            "WARNING: no sign configured for device serial " ..
            my_serial
        )
    end

    print(
        "my screen indy id is " ..
        tostring(indy_id) ..
        ", rotation is " ..
        tostring(rotation)
    )
    ...
    st =
        util.screen_transform(rotation)
    ...
    if portrait == false and WIDTH > HEIGHT then
        print(
            "WARNING: landscape canvas (" ..
            WIDTH ..
            "x" ..
            HEIGHT ..
            ") - portrait-mounted signs need rotation 90 or 270"
        )
    end
...
local function draw_hugged_poster(
    media_w,
    media_h,
    draw_media
)
...
local function Fallback(asset_name, duration)
    local object =
        resource.load_image(asset_name)
...
local function Image(asset_name, duration)
    ...
    local function draw()
        ...
        -- Draw logo.png above the poster.
        draw_top_logo()
...
local function Video(asset_name)
    ...
    local function draw()
        ...
        -- Draw logo.png above the video/poster area.
        draw_top_logo()

        if not object then
            object = resource.load_video{
                file = file;
                raw = true;
            }
        end

        local state, video_w, video_h =
            object:state()
...
local function Player()
    ...
    local function asset_key()
        if not screen.show or
           screen.show.poster_file == "" then
        ...
        return
            (screen.show.media_type or "image") ..
            ":" ..
            screen.show.poster_file
    end
...
function node.render()
    ...
    if debug then
        ...
        font:write(
            x,
            y,
            ("Footer: y=%d h=%d"):format(
                layout.footer_y or -1,
                layout.footer_h or -1
            ),
            12,
            1, 1, 1, 1
        )
